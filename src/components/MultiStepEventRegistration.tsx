...
export default function MultiStepEventRegistration({
  ...
  const handleUpsellChange = (upsellId: string, quantity: number) => {
    setSelectedUpsells((previous) => {
      const upsell = upsells.find((item) => item.id === upsellId)
      const maxAllowed =
        upsell?.type === 'tshirt' || upsell?.type === 'photos' ? selectedTicketSlots.length : Number.MAX_SAFE_INTEGER
  ...
  const handleUpsellAssignmentToggle = (upsellId: string, ticketId: string, checked: boolean) => {
    setSelectedUpsells((previous) => {
      ...
      const current: string[] = Array.isArray(entry.meta?.assignments) ? (entry.meta!.assignments as string[]) : []
  ...
  const renderOptionsStep = () => (
    <div className="space-y-4">
      ...
      {upsells.map((upsell) => {
        ...
        const maxQuantityForUpsell = (upsell.type === 'tshirt' || upsell.type === 'photos') ? selectedTicketSlots.length : undefined
        const addDisabled =
              ...
              <div className="flex flex-col items-end gap-3">
                ...
                {upsell.type === 'photos' && quantity > 0 ? (
                  <div className="flex w-full flex-col items-end gap-2 text-sm">
                    <div className="text-xs text-muted-foreground">Assigner le pack photo (1 par billet):</div>
                    <div className="grid w-full grid-cols-1 gap-2 sm:grid-cols-2">
                      {participants.map((p, idx) => {
                        ...
                        return (
                          <label key={`${upsell.id}-assign-${p.id}`} className={`flex items-center gap-2 rounded border p-2 ${checked ? 'border-primary' : 'border-border'}`}>
                            <input
                              type="checkbox"
                              className="h-4 w-4"
                              checked={checked}
                              disabled={disableUnchecked}
                              onChange={(e) => handleUpsellAssignmentToggle(upsell.id, p.ticketId, e.target.checked)}
                            />
                            <span>
                              Participant #{idx + 1}{p.firstName || p.lastName ? ` — ${p.firstName} ${p.lastName}` : ''}
                            </span>
                          </label>
                        )
                      })}
      ...
      if (upsell.type === 'photos') {
        ...
        if (assignments.length) {
          const labels = assignments.map((ticketId) => {
            ...
            return p && (p.firstName || p.lastName) ? `${p.firstName ?? ''} ${p.lastName ?? ''}`.trim() || `Participant #${index + 1}` : `Participant #${index + 1}`
          })
